...
def get_today_token_usage_breakdown(user_id):
    ...
    try:
        ...
        for log in today_logs:
            try:
                if log.details and "additional tokens" in log.details.lower():
                    # Parse details like "Used 2 from daily quota + 3 additional tokens (Total: 5)"
                    import re
                    
                    # Look for "X additional tokens" pattern
                    ...
                    # Also extract total tokens for verification
                    total_match = re.search(r'Total:\s*(\d+)', log.details, re.IGNORECASE)
                    if total_match:
                        total_tokens_from_logs += int(total_match.group(1))
                    
                elif log.details and "Tokens used:" in log.details:
                    # Fallback for older log format
                    ...
                    if token_match:
                        ...
                        # If more tokens were used than daily quota allows, 
                        # the excess must be additional tokens
                        if total_tokens_from_logs > daily_limit:
                            additional_tokens_used += min(tokens_in_log, total_tokens_from_logs - daily_limit)
                
            except Exception as e:
                app.logger.error(f"Error parsing usage log {log.id}: {str(e)}")
                continue
        ...
        app.logger.error(f"Error calculating today's token usage: {str(e)}")
        return {
            'daily_quota_used': 0,
            'additional_tokens_used': 0,
            'total_tokens_used_today': 0,
...
def index():
    ...
    daily_quota_used = 0  # Initialize here
    additional_tokens_used_today = 0  # Initialize here
    next_milestone = 100  # Initialize here
    ...
    if user_id:
        ...
        if active_subscription:
            ...
            today_token_usage = today_usage['total_tokens_used_today']  # ✅ Now includes additional tokens
            ...
            for log in yesterday_logs:
                try:
                    if log.details and "Total:" in log.details:
                        import re
                        match = re.search(r'Total:\s*(\d+)', log.details)
                        if match:
